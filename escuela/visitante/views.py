...
class RecomendacionCreateView(CreateView):
	...
	def post(self,request,*args,**kwargs):
		...
		if form.is_valid():
			...
			return render(request, 'visitante/recomendacion_form.html', {'form': form,'curso':curso_post,'titulo_template': 'Registro de una recomendacion'})  
...
class TemaCreateView(CreateView):
	...
	def get_context_data(self,**kwargs):
		...
		context['titulo'] = str(Capitulo.nombre) + ' ' + str(Capitulo.curso.titulo)
		return context
...
class TemaUpdateView(UpdateView):
	...
	def get_context_data(self, **kwargs):
		...
		Capitulo = capitulo.objects.get(id= self.request.GET['capitulo_id'])
		context['capitulo'] = Capitulo
		context['titulo_template'] = 'Edicion Tema'
		context['titulo'] = str(Capitulo.nombre) + ' ' + str(Capitulo.curso.titulo)
		# Detalles
